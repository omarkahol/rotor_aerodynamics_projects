function sol = forewardFlightSolver(data,solver)
    ...
    for n = 3:solver.NT*solver.Sweeps
        ...
        while err > solver.Tol && it < solver.ItMax
            ...
            sol.alfa_e(n,:) = theta(n) + (r./data.mu).*sol.dbeta(n);
            ...
            ddalfa_e = (sol.alfa_e(n)-2*sol.alfa_e(n-1)+sol.alfa_e(n-2))...
                ./(solver.dt^2);
            %Cl = Cl + 0.5*pi*data.cr.*(dalfa_e./(solver.dt.*sol.v(n,:).^2)+...
            %    r.*sol.ddbeta(n-1)./(sol.v(n,:).^2) - ...
        ...
        if n ~= solver.NT*solver.Sweeps
            sol.ddbeta(n) = (3/data.mass)*sum((r(1:index)-data.r0).*ct(1:index)) - ...
                3*sin(sol.beta(n))*sum((r-data.r0).*r.*data.dr);            
